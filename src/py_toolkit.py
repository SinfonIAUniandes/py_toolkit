...
class PyToolkit:
    ...
    def __init__(self, session):
        ...
        self.segmentation3DGetSegmentation3DServer = rospy.Service(
            "pytoolkit/ALSegmentation3D/get_segmentation3D_srv",
            get_segmentation3D_srv,
            self.callback_segmentation3D_get_segmentation3D_srv,
        )
        ...
        self.tabletShowImageServer = rospy.Service(
            "pytoolkit/ALTabletService/show_image_srv",
            tablet_service_srv,
            self.callback_tablet_show_image_srv,
        )
        print(consoleFormatter.format("Show_image_srv on!", "OKGREEN"))

        self.tabletShowWebViewServer = rospy.Service(
            "pytoolkit/ALTabletService/show_web_view_srv",
            tablet_service_srv,
            self.callback_tablet_show_web_view_srv,
        )
        print(consoleFormatter.format("Show_web_view_srv on!", "OKGREEN"))

        self.tabletShowTopicServer = rospy.Service(
            "pytoolkit/ALTabletService/show_topic_srv",
            tablet_service_srv,
            self.callback_tablet_topic_srv,
        )
        print(consoleFormatter.format("Show_topic_srv on!", "OKGREEN"))

        self.tabletPlayVideoServer = rospy.Service(
            "pytoolkit/ALTabletService/play_video_srv",
            tablet_service_srv,
            self.callback_tablet_play_video_srv,
        )
        print(consoleFormatter.format("Play_video_srv on!", "OKGREEN"))

        self.tabletGetInputServer = rospy.Service(
            "pytoolkit/ALTabletService/get_input_srv",
            get_input_srv,
            self.callback_tablet_get_input_srv,
        )
        print(consoleFormatter.format("Get_input_srv on!", "OKGREEN"))

        self.tabletShowWordsServer = rospy.Service(
            "pytoolkit/ALTabletService/show_words_srv",
            battery_service_srv,
            self.callback_tablet_show_words_srv,
        )
        print(consoleFormatter.format("Show_words_srv on!", "OKGREEN"))

        self.tabletShowPicturesServer = rospy.Service(
            "pytoolkit/ALTabletService/show_picture_srv",
            battery_service_srv,
            self.callback_tablet_show_picture_srv,
        )
        print(consoleFormatter.format("Show_picture_srv on!", "OKGREEN"))

        self.tabletHideServer = rospy.Service(
            "pytoolkit/ALTabletService/hide_srv",
            battery_service_srv,
            self.callback_tablet_hide_srv,
        )
        print(consoleFormatter.format("Hide_srv on!", "OKGREEN"))

        self.enableSecurityServer = rospy.Service(
            "pytoolkit/ALMotion/enable_security_srv",
            battery_service_srv,
            self.callback_motion_enable_security_srv,
        )
        print(consoleFormatter.format("enable_security_srv on!", "OKGREEN"))

        self.tabletOverloadServer = rospy.Service(
            "pytoolkit/ALTabletService/overload_srv",
            battery_service_srv,
            self.callback_tablet_overload_srv,
        )
        print(consoleFormatter.format("Overload_srv on!", "OKGREEN"))

        self.toggleAppLauncherServer = rospy.Service(
            "pytoolkit/ALServiceManager/toggle_applauncher_srv",
            SetBool,
            self.callback_toggle_applauncher_srv,
        )
    ...
    def callback_tablet_topic_srv(self, req):
        ...
        try:
            self.ALTabletService.showWebview(
                "http://" + ip + ":8080/stream_viewer?topic=" + req.url
            )
            time.sleep(3)
            script = """
    ...
    def callback_tablet_get_input_srv(self, req):
        ...
        try:
            self.ALTabletService.showWebview(
                "http://198.18.0.1/apps/robot-page/" + html_file + ".html"
            )
            script = (
                open(self.PYTOOLKIT_FOLDER + "/resources/" + code_file + ".txt", "r")
                .read()
                .replace("+++++", req.text)
            )
            time.sleep(1)
    ...
    def callback_tablet_show_picture_srv(self, req):
        ...
        self.showing_words = False
        self.ALPhotoCapture.takePicture(
            "/home/nao/.local/share/PackageManager/apps/robot-page/html/img/",
            "picture",
            True,
        )
        rospy.sleep(2)
        try:
            self.ALTabletService.showImageNoCache(
                "http://198.18.0.1/apps/robot-page/img/picture.jpg"
            )
        except:
            self.ALTabletService = session.service("ALTabletService")
            self.ALTabletService.showImageNoCache(
		...
		displayWords(1);
            ...
            while "pau=" in nuevo_string:
                ...
                if index_pau > 0:  # Verifica si "pau=" no está al principio
                    index_backslash = nuevo_string.find(
                        "\\", index_pau
                    )  # Encuentra el índice del primer backslash después de "pau="
                    nuevo_string = (
                        nuevo_string[: index_pau - 2]
                        + nuevo_string[index_backslash + 1 :]
                    )
                ...
                if "\\rspd=" in nuevo_string:
                    ...
                    nuevo_string = (
                        nuevo_string[:index_rspd] + nuevo_string[index_backslash + 1 :]
                    )
            if "rspd" in nuevo_string:
                for i, caracter in enumerate(
                    msg.text.replace("\\", "").replace("rspd=", "")
                ):
                    if not caracter.isdigit():
                        nuevo_string = msg.text.replace("\\", "").replace("rspd=", "")[
                            i:
                        ]
                        break
...
if __name__ == "__main__":
    ...
    try:
        ...
        time.sleep(1)
        pytoolkit.ALTabletService.showImage(
            "http://198.18.0.1/apps/robot-page/img/logo.png"
        )
        print(
            consoleFormatter.format(
                " \n----------------------------------------------------------",
